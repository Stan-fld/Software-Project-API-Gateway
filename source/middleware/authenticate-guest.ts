...
export function authenticateGuest(req: any, res: any, next: any) {
    ...
    User.findWithAccessToken(token).then((user: User) => {
        ...
        } else if (!isTheRole(user, roles.guest)) {
            return Promise.reject(authenticationFailed('An guest account is required to access these endpoints, access is denied.'));
        }
